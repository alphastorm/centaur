        ...
        match kind {
            ...
            "tool_execution_end" => {
                ...
                events.push(NormalizedEvent::ToolResults(vec![NormalizedToolResult {
                    tool_use_id,
                    content: bounded_text(&render_result(&result)),
                    is_error: frame
                        .get("isError")
            ...
            "auto_retry_end" => {
                if frame.get("success").and_then(Value::as_bool) == Some(false)
                    && let Some(error) = frame.get("finalError").and_then(Value::as_str)
                {
            ...
            Some("thinking" | "reasoning") => {
                    ...
                    .and_then(Value::as_str)
                {
                    normalized.push(NormalizedContent::ReasoningText {
                        item_id: format!("{fallback_id}-reasoning-{index}"),
                        text: bounded_text(text),
                    });
                }
            }
